...
class ResNext(Pruneable):

    def __init__(self, device="cuda", output_dim=2, input_dim=(1, 1, 1), **kwargs):
        super(ResNext, self).__init__(device=device, output_dim=output_dim, input_dim=input_dim, **kwargs)

        self.m = torch.hub.load('facebookresearch/WSL-Images', 'resnext101_32x48d_wsl')
